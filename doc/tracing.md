# User-space, Statically Defined Tracing (USDT) for Renaiss Core

Renaiss Core includes statically defined tracepoints to allow for more
...
**Note**: Renaiss Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Renaiss Core
...
Multiple tools can list the available tracepoints in a `renaissd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Renaiss Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/renaissd
stap net        outbound_message 0x0000000000107c05 /src/renaissd
stap validation block_connected  0x00000000002fb10c /src/renaissd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Renaiss Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/renaissd | grep NT_STAPSDT -A 4 -B 2
