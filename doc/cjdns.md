# CJDNS support in Renaiss Core

It is possible to run Renaiss Core over CJDNS, an encrypted IPv6 network that
...
done outside of Renaiss Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Renaiss network and individual nodes.
...
before it will work with your Renaiss Core node. This is described in steps
...
## Run Renaiss Core with CJDNS

Once you are connected to the CJDNS network, the following Renaiss Core
...
When enabled, this option tells Renaiss Core that it is running in an
...
IPv6 local network. This helps Renaiss Core perform better address management:
...
CJDNS support was added to Renaiss Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `renaiss-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/renaiss/renaiss/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Renaiss Core

There are several ways to see your CJDNS address in Renaiss Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `renaiss-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `renaiss-cli getpeerinfo`).
