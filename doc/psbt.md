# PSBT Howto for Renaiss Core

Since Renaiss Core 0.17, an RPC interface exists for Partially Signed Renaiss
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/renaiss/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Renaiss transactions that are not fully signed
...
#### Multisig with multiple Renaiss Core instances
...
Renaiss Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `renaiss-cli` is used.
...
Later, when *V* REN has been received on *Amulti*, and Bob and Carol want to
