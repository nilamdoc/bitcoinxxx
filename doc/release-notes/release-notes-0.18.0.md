Renaiss Core version 0.18.0 is now available from:

  <https://renaisscore.org/bin/renaiss-core-0.18.0/>
...
`/Applications/Renaiss-Qt` (on Mac) or `renaissd`/`renaiss-qt` (on
Linux).
...
recommended to use Renaiss Core on unsupported systems.
...
10.10. Additionally, Renaiss Core does not yet change appearance when
...
`contrib/init/renaissd.service`, it has been changed to use
`/var/lib/renaissd` as the data directory instead of
`~renaiss/.renaiss`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/renaissd` will
exist has enough space (check using `df -h /var/lib/renaissd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/renaiss/renaiss/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://renaisscore.org/en/contact/) the Renaiss Core
...
The systemd init file (`contrib/init/renaissd.service`) has been changed
to use `/var/lib/renaissd` as the data directory instead of
`~renaiss/.renaiss`. This change makes Renaiss Core more consistent with
...
`datadir` option in `/etc/renaiss/renaiss.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/renaiss/renaiss.conf`.


Documentation
-------------

- A new short [document](https://github.com/renaiss/renaiss/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/renaiss/renaiss/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/renaiss/renaiss/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/renaiss/renaiss/blob/master/doc/renaiss-conf.md)
  about the `renaiss.conf` file describes how to use it to configure
  Renaiss Core.

- A new document introduces Renaiss Core's BIP174 [Partially-Signed
  Renaiss Transactions
  (PSBT)](https://github.com/renaiss/renaiss/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/renaiss/renaiss/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Renaiss-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Renaiss Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/renaiss/renaiss/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/renaiss/renaiss/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Renaiss Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Renaiss Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/renaiss/renaiss/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/renaiss/renaiss/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart renaissd with the `-deprecatedrpc=generate`
...
- A new `renaiss-wallet` tool is now distributed alongside Renaiss
...
This section describes planned changes to Renaiss Core that may affect
other Renaiss software and services.

- Since version 0.16.0, Renaiss Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Renaiss Core 0.20 (expected about
  a year after 0.18), Renaiss Core will default to native segwit
  ...
  to bech32 addresses, and if the Renaiss Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Renaiss Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Renaiss Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Renaiss Core
...
- A sub-project of Renaiss Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Renaiss Core.  See their [project
  page](https://github.com/renaiss-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Renaiss Core's own implementation, although entropy
  gathered by Renaiss Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Renaiss Core a
...
- On macOS, Renaiss Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `renaiss_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `librenaiss_server.a` (jnewbery)
...
- #13926 [Tools] renaiss-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like renaissd (MarcoFalke)
...
- #14521 qt, docs: Fix `renaiss-qt -version` output formatting (hebasto)
...
- #11625 Add RenaissApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `librenaiss_server` (laanwj)
...
- #15154 Configure: renaiss-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/renaiss-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Renaiss Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `librenaiss_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in renaiss-util-test.py (isghe)
...
- #12775 Integration of property based testing into Renaiss Core (Christewart)
...
- #12246 Bugfix: Only run renaiss-tx tests when renaiss-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_renaiss` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_renaiss` (MarcoFalke)
...
- #14186 renaiss-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_renaiss.conf` message on startup if conf file exists (leishman)
...
- #12255 Update renaiss.service to conform to init.md (dongcarl)
...
- #14497 Add doc/renaiss-conf.md (hebasto)
...
- #15354 Add missing `renaiss-wallet` tool manpages (MarcoFalke)
...
- #15765 correct renaissconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support renaiss folder on OSX (JimmyMow)
