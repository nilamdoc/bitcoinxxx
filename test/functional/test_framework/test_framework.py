#!/usr/bin/env python3
# Copyright (c) 2014-2021 The Renaiss Core developers
...
TMPDIR_PREFIX = "renaiss_func_test_"
...
class RenaissTestMetaClass(type):
    """Metaclass for RenaissTestFramework.

    Ensures that any attempt to register a subclass of `RenaissTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'RenaissTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("RenaissTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("RenaissTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class RenaissTestFramework(metaclass=RenaissTestMetaClass):
    """Base class for a renaiss test script.

    Individual renaiss test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave renaissds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop renaissds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use renaiss-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_renaissd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "renaissd" + config["environment"]["EXEEXT"],
        )
        fname_renaisscli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "renaiss-cli" + config["environment"]["EXEEXT"],
        )
        fname_renaissutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "renaiss-util" + config["environment"]["EXEEXT"],
        )
        self.options.renaissd = os.getenv("RENAISSD", default=fname_renaissd)
        self.options.renaisscli = os.getenv("RENAISSCLI", default=fname_renaisscli)
        self.options.renaissutil = os.getenv("RENAISSUTIL", default=fname_renaissutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: renaissds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("RenaissRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'renaissd', self.options.renaissd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'renaiss-cli', self.options.renaisscli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.renaissconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a renaissd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple renaissd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as renaissd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    renaissd=self.options.renaissd,
                    renaiss_cli=self.options.renaisscli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in renaiss.conf
    ...
    def skip_if_no_renaissd_tracepoints(self):
        """Skip the running test if renaissd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("renaissd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_renaissd_zmq(self):
        """Skip the running test if renaissd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("renaissd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if renaiss-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("renaiss-wallet has not been compiled")

    def skip_if_no_renaiss_util(self):
        """Skip the running test if renaiss-util has not been compiled."""
        if not self.is_renaiss_util_compiled():
            raise SkipTest("renaiss-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if renaiss-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("renaiss-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether renaiss-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether renaiss-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_renaiss_util_compiled(self):
        """Checks whether renaiss-util was compiled."""
        return self.config["components"].getboolean("ENABLE_RENAISS_UTIL")
