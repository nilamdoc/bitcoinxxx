# Bootstrappable Renaiss Core Builds

This directory contains the files necessary to perform bootstrappable Renaiss
Core builds.
...
To build Renaiss Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Renaiss Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/renaiss-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/renaiss-core/guix.sigs).
