...
""" Interactive renaissd P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Renaiss Core can enable. It uses BCC
...
def main(renaissd_path):
    peers = dict()

    renaissd_with_usdts = USDT(path=str(renaissd_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    renaissd_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    renaissd_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[renaissd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/renaissd")
        exit()
    path = sys.argv[1]
    main(path)
