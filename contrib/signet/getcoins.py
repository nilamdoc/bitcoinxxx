...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing renaiss-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='renaiss-cli', help='renaiss-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Renaiss address to which the faucet should send')
...
parser.add_argument('renaiss_cli_args', nargs='*', help='Arguments to pass on to renaiss-cli (default: -signet)')

args = parser.parse_args()

if args.renaiss_cli_args == []:
    args.renaiss_cli_args = ['-signet']


def renaiss_cli(rpc_command_and_params):
    argv = [args.cmd] + args.renaiss_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = renaiss_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = renaiss_cli(['getnewaddress', 'faucet', 'bech32'])
