...
const char * const RENAISS_CONF_FILENAME = "renaiss.conf";
const char * const RENAISS_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //renaiss-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", RENAISS_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Renaiss
    // macOS: ~/Library/Application Support/Renaiss
    // Unix-like: ~/.renaiss
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Renaiss";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Renaiss";
#else
    // Unix-like
    return pathRet / ".renaiss";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", RENAISS_CONF_FILENAME);
