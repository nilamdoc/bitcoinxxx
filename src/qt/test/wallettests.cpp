// Copyright (c) 2015-2021 The Renaiss Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<RenaissAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = RenaissUnits::formatWithUnit(unit, expected_balance, false, RenaissUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_renaiss-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_renaiss-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_renaiss-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    ...
    // Amount input
    RenaissAmountField* amountInput = receiveCoinsDialog.findChild<RenaissAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_renaiss-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
