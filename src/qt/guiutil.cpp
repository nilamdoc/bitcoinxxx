...
#include <qt/renaissaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Renaiss address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new RenaissAddressEntryValidator(parent));
    widget->setCheckValidator(new RenaissAddressCheckValidator(parent));
}
...
bool parseRenaissURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no renaiss: URI
    if(!uri.isValid() || uri.scheme() != QString("renaiss"))
        return false;
...
bool parseRenaissURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseRenaissURI(uriInstance, out);
}

QString formatRenaissURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("renaiss:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openRenaissConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", RENAISS_CONF_FILENAME));
    ...
    /* Open renaiss.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Renaiss.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Renaiss (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Renaiss (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Renaiss*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "renaiss.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("renaiss-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a renaiss.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Renaiss\n";
        else
            optionFile << strprintf("Name=Renaiss (%s)\n", chain);
