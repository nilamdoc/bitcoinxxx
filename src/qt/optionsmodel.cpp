...
/** Call node.updateRwSetting() with Renaiss 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Renaiss 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/renaiss/renaiss/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because renaiss 22.x will not try to read these.
...
//! Convert enabled/size values to renaiss -prune setting.
...
//! Get pruning enabled value to show in GUI from renaiss -prune setting.
...
//! Get pruning size value to show in GUI from renaiss -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if renaiss.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_renaiss_unit);
...
{
    if (new_unit.isNull() || new_unit.value<RenaissUnit>() == m_display_renaiss_unit) return;
    m_display_renaiss_unit = new_unit.value<RenaissUnit>();
    QSettings settings;
    settings.setValue("DisplayRenaissUnit", QVariant::fromValue(m_display_renaiss_unit));
    Q_EMIT displayUnitChanged(m_display_renaiss_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/renaiss/renaiss/pull/8273
    ...
    // (https://github.com/renaiss-core/gui/issues/567).
