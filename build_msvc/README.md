Building Renaiss Core with Visual Studio
========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build Renaiss Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Renaiss Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build Renaiss Core with the GUI, a static build of Qt is required.
...
To build Renaiss Core without Qt, unload or disable the `renaiss-qt`, `librenaiss_qt` and `test_renaiss-qt` projects.
...
```cmd
msbuild build_msvc\renaiss.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/renaiss.sln` file in Visual Studio.

Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make Renaiss Core more secure. When building Renaiss using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `renaissd` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/renaissd.exe
```
