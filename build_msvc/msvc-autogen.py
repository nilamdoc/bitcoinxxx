...
def parse_config_into_ren_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Renaiss Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/renaiss_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/renaiss_config.h'), "w", encoding="utf8") as ren_config:
        ren_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Renaiss-core msbuild configuration initialiser.')
    ...
    parse_config_into_ren_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/renaiss_config.h'), os.path.join(SOURCE_DIR, 'config/renaiss-config.h'))
